  ...
  return (
    <div className="w-full h-full bg-gray-50">
      <div className="py-8 px-4 sm:px-6 lg:px-8">
        <h1 className="text-2xl font-bold text-minhagrana-primary mb-6">Relatório Detalhado</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6 min-h-[300px]">
            ...
            <div className="flex items-center justify-center h-48 text-gray-400">[Gráfico de barras detalhado aqui]</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6 min-h-[300px]">
            <div className="font-bold text-lg mb-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="font-bold text-lg mb-4">Receitas Detalhadas</div>
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {(transacoes.filter(t => t.tipo === 'entrada').concat(futuros.filter(t => t.tipo === 'entrada'))).map((t, idx) => (
                    <tr key={idx}>
                      <td className="px-2 py-1">{t.data ? new Date(t.data).toLocaleDateString('pt-BR') : t.data_prevista ? new Date(t.data_prevista).toLocaleDateString('pt-BR') : '-'}</td>
                      <td className="px-2 py-1">{t.descricao || '-'}</td>
                      <td className="px-2 py-1">{Number(t.valor || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
                      <td className="px-2 py-1">{t.data ? 'Transação' : 'Lançamento Futuro'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="font-bold text-lg mb-4">Despesas Detalhadas</div>
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="bg-gray-100">
                ...
                <tbody>
                  {(transacoes.filter(t => t.tipo === 'saida').concat(futuros.filter(t => t.tipo === 'saida'))).map((t, idx) => (
                    <tr key={idx}>
                      <td className="px-2 py-1">{t.data ? new Date(t.data).toLocaleDateString('pt-BR') : t.data_prevista ? new Date(t.data_prevista).toLocaleDateString('pt-BR') : '-'}</td>
                      <td className="px-2 py-1">{t.descricao || '-'}</td>
                      <td className="px-2 py-1">{Number(t.valor || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td>
